 ...
 * socket table (returned by some functions defined here).
 */
static int getSocketProp(struct element_table *table, char *key)
{
    struct element *prop = getTrueElement(elementGet(key, table, NULL));
...
{
    SOCKET sock = (int) getSocketProp(getValueTable(args->first), "_SOCK");
...
{
    struct element_table *sock = getValueTable(args->first);
...
{
    const int SOCK = getSocketProp(getValueTable(args->first), "_SOCK");
...
{
    struct element_table *result, *sock = getValueTable(args->first);
    struct element *prop;
...
void stdClose(struct result_list *args)
{
    ...
    WSACleanup();
#endif
    if (closesocket(getSocketProp(getValueTable(args->first), "_SOCK")) == SOCKET_ERROR) {
        setLastError(errno);
    }
}
...
void stdShutdown(struct result_list *args)
{
    ...
    int result = shutdown(getSocketProp(getValueTable(args->first), "_SOCK"), how);
...
{
    SOCKET sock = getSocketProp(getValueTable(args->first), "_SOCK");
